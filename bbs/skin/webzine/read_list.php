...
<?php
echo get_paging($config['write_pages'], $page, $total_page, TB_BBS_URL.'/list.php?'.$qstr3.'&page=');
?>
...
<div class="tbl_frm01 tbl_wrap mart30">
	...
	$sql .= " order by fid asc, thread asc limit 1";
	$res = sql_query($sql);
	if(sql_num_rows($res)) {
	...
	if($sfl && $stx)
		$sql .= " and $sfl like '%$stx%'";
	$sql .= " order by fid desc, thread asc limit 1";
	$res = sql_query($sql);
	if(sql_num_rows($res)) {
		$row = sql_fetch_array($res);

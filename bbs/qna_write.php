...
if($_POST['mode'] == 'w') {
	...
	if($email) {
		...
		mailer($wr_name, $email, $mk['email'], $subject, $content, 1);
	}

	// 운영자에게 문자전송
	icode_direct_sms_send($pt_id, $mk['cellphone'], $subject);

	alert("정상적으로 접수 되었습니다. 빠른 시간안에 답변드리겠습니다", TB_BBS_URL."/qna_list.php");
}
...
$form_action_url = TB_HTTPS_BBS_URL.'/qna_write.php';
include_once(TB_THEME_PATH.'/qna_write.skin.php');

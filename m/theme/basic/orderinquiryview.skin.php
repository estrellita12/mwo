...
<div id="sod_fin">
	...
	<section id="sod_fin_list">
        ...
        <ul id="sod_list_inq" class="sod_list">
			...
			for($i=0; $rw=sql_fetch_array($result); $i++) {
				...
				unset($it_name);
				$it_options = mobile_print_complete_options($rw['gs_id'], $rw['od_id']);
				if($it_options){
				...
				<td>
					...
					if($od['paymethod'] == '휴대폰')
					{
						if($od['od_pg'] == 'lg') {
							...
							$hp_receipt_script = 'window.open(\'https://npg.nicepay.co.kr/issue/IssueLoader.do?TID='.$od['od_tno'].'&&type=0\',\'popupIssue\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes, resizable=yes,width=420,height=540\');';
						}
					?>
			...
			<tr>
				...
				if($od['od_cash'])
				{
					if($od['od_pg'] == 'lg') {
						...
						$cash = unserialize($od['od_cash_info']);
						$cash_receipt_script = 'window.open(\'https://npg.nicepay.co.kr/issue/IssueLoader.do?TID='.$cash['TID'].'&type=1\',\'popupIssue\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=414,height=622\');';
					}
				?>

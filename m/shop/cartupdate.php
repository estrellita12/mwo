...
if($act == "buy")
{
    ...
    if($is_member) // 회원인 경우
		...
		goto_url(TB_MBBS_URL."/login.php?url=".urlencode("/m/shop/orderform.php"));
}
	...
	for($i=0; $i<$count; $i++) {
		...
		for($k=0; $k<$opt_count; $k++) {
			...
			$sql = " insert into shop_cart
					...
					VALUES ";
			$sql.= "( '$ca_id', '{$member['id']}', '{$gs['index_no']}', '$set_cart_id', '".TB_TIME_YMDHIS."', '{$gs['goods_price']}', '{$gs['supply_price']}', '$ct_qty', '{$gs['gpoint']}', '$io_id', '$io_type', '$io_supply_price', '$io_price', '$io_value', '$ct_send_cost', '$od_no', '{$_SERVER['REMOTE_ADDR']}' )";
			sql_query($sql);

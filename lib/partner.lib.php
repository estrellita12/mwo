...
// 가맹점 판매수수료 적립 대상
function set_item_commission_id($pt_id)
{
	global $config, $is_member, $member;

	$mb_recommend = $pt_id;

	if($is_member) { // 회원일때
		if(is_partner($member['id'])) {
			if($config['pf_sale_target2'] == '1')
				$mb_recommend = $member['pt_id']; // 주문자가 가맹점일 경우 현재 추천인에게 판매수수료가 적립
			else if($config['pf_sale_target2'] == '2')
				$mb_recommend = $member['id']; // 주문자가 가맹점일 경우 가맹점 자신에게 판매수수료가 적립
		} else {
			if($config['pf_sale_target1'] == '1')
				$mb_recommend = $member['pt_id']; // 주문자가 회원일 경우 현재 추천인에게 판매수수료가 적립
		}
	}

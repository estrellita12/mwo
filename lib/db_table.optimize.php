...
if($config['cf_visit_del'] > 0) {
    $tmp_before_date = date("Y-m-d", TB_SERVER_TIME - ($config['cf_visit_del'] * 86400));
    $sql = " delete from shop_visit where vi_date < '$tmp_before_date' ";
    sql_query($sql);
    sql_query(" OPTIMIZE TABLE `shop_visit`, `shop_visit_sum` ");
}

// 설정일이 지난 인기검색어 삭제
if($config['cf_popular_del'] > 0) {
    $tmp_before_date = date("Y-m-d", TB_SERVER_TIME - ($config['cf_popular_del'] * 86400));
    $sql = " delete from shop_popular where pp_date < '$tmp_before_date' ";
    sql_query($sql);
    sql_query(" OPTIMIZE TABLE `shop_popular` ");
}

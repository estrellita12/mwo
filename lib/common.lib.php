...
// get_listtype_skin_extra - mwo simple 버전
function get_listtype_skin_extra($type, $width, $height, $rows, $li_css='')
{
    global $pt_id;

    $str = "";
    $result = display_itemtype_extra($pt_id, $type, $rows);
    for($i=0; $row=sql_fetch_array($result); $i++)
    {
        if($i==0)
            $str .= "<div class=\"pr_desc {$li_css}\">\n<ul>\n";
...
function get_order_seller_id($seller_id)
{
    if($seller_id == 'admin') {
        ...
        $mb = get_member($sr['mb_id'], 'name');
        $disp_sr_id = get_sideview($seller_id,$mb['name']);
    } else {
...
// 2023-10-12
function admin_order_status_cnt($where)
{
    $sql = " select count(od_id) as cnt from shop_order {$where} ";
    $row = sql_fetch($sql);
    $od_count = (int)$row['cnt'];
...
function get_price($gs_id, $msg='<span>원</span>')
{
    ...
    $isopen_chk = sql_fetch("SELECT isopen FROM shop_goods WHERE index_no = {$gs_id}");//isopen 확인하여 상태값 변경 추가
    $isopen_config = ['', '진열','품절','단종','중지'];
    $stat_text = $isopen_config[$isopen_chk['isopen']];

    if(is_soldout($gs['index_no'])) {
        $str = "<span class=\"soldout\">{$stat_text}</span>";
    } else {
        if($gs['price_msg']) {
            $str = $gs['price_msg'];
...
function get_change_select($name, $selected='', $event='')
{
    ...
    if(
        ...
        $_SESSION['ss_mb_id']=='MDadmin' || 
        $_SESSION['ss_mb_id'] =='admin1' || 
        $_SESSION['ss_mb_id'] =='admin12' || 
        $_SESSION['ss_mb_id'] =='admin16' || 
        $_SESSION['ss_mb_id'] == 'admin18' || //accesable for temporarily.. need checking
        $_SESSION['ss_mb_id'] == 'admin14' ||
        $_SESSION['ss_mb_id'] == 'admin15' ||
        $_SESSION['ss_mb_id'] == 'admin17' ||
        $_SESSION['ss_mb_id'] == 'admin11' ||
        $_SESSION['ss_mb_id'] == 'admin19' ||
        $_SESSION['ss_mb_id'] == 'admin13'        
    ){
        $str = "<select id=\"order_stat_box\" name=\"{$name}\"{$event}>\n";
        foreach($gw_status as $key=>$val) {
    ...
    if( in_array($selected, array(6,7,8,9))) {
    ...
    foreach($gw_array_status as $key=>$val) {
        ...
        // 공급사 페이지 주문상세 팝업 창을 통한 접근에서 상태값 4,5에 대하여 9노출 예외처리 2023-12-28
        /*(예외처리 삭제 시 seller_odr_statusupdate.php 내 9에 대한 허용 배열 내 '4,5' 삭제 필요)*/
        if(strpos($_SERVER["REQUEST_URI"],'seller_odr_form') && ($selected == '4' || $selected == '5')) $str .= option_selected('9', '', $gw_status['9']);
    }
    $str .= "</select>";
    return $str;
}
...
// 주문완료 옵션호출
function print_complete_options_bak($gs_id, $od_id, $xls='')
{
...
function print_complete_options($gs_id, $od_id, $xls='')
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if(!$xls) {
            ...
            if($row['io_type'])
                $str .= "<li class='ny'>".$row['ct_option']." (".$price_plus.display_price($row['io_price']).")</li>".PHP_EOL;
            else
                $str .= "<li class='ty'>".$row['ct_option']." (".$price_plus.display_price($row['io_price']).")</li>".PHP_EOL;
        } else {

            $str .= $comma.$row['ct_option']." (".$price_plus.display_price($row['io_price']).")".PHP_EOL;
...
function get_category_select_1($name, $selected, $event='')
{
    ...
    if(!is_admin()){ // 2023-02-14
        $sql = "select * from shop_category where length(catecode)='3' and cateuse=0 order by caterank, catecode";
    }
    $res = sql_query($sql);
...
function get_seller_name($mb_id)
{
    ...
    if(substr($mb_id,0,3) == 'AP-') {
        ...
        $row = sql_fetch("select mb_id from shop_seller where seller_code = '$mb_id'");
        $seller_id = $row['mb_id'];
        $row2 = sql_fetch("select name from shop_member where id = '$row[mb_id]'");
...
// 배열에 대상을 추가
function set_items_array_add($str, $mb_id)
{
    $str = preg_replace('/\s+/', '', $str);

    $list_data = explode(",", $str);
...
// 배열에 대상을 제거
function set_items_array_del($str, $mb_id)
{
    $str = preg_replace('/\s+/', '', $str);

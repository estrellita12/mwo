...
// 상품 진열 extra - mwo simple 버전
function display_itemtype_extra($mb_id, $type, $rows='')
{
    //공급사 체크
    $login_id = get_session('ss_mb_id');
    $chk_sql = "select seller_code from shop_seller where mb_id = '{$login_id}'";
    $chk_result = sql_fetch($chk_sql);    
    $seller_option = $chk_result ? "and mb_id = '{$chk_result[seller_code]}'" : "";
/* 상품 공급상태에 따라 안보이게
	$sql = " select *
			   from shop_goods
			  where shop_state = '0'
...
function is_seller($mb_id)
{
    ...
    if($config['shop_state'] == 0 && ($sr['state']==0 || $sr['state']==1 )) {
		return true;
	} else {
		return false;
	}
...
// 공급업체 유일키검사
function code_uniqid() // 2024-01-04
{
...
function get_order_spay($od_id, $sql_search='')
{
	$sql = " select SUM(goods_price) as price,
					...
					SUM(baesong_price2) as baesong2,
...
function change_order_status_1($od_no)
{
	$sql = " update shop_order
				set dan = '1'
                  ...
                  , rcent_time = '".TB_TIME_YMDHIS."'
			  where od_no = '$od_no'
			    and dan = '2' ";
	sql_query($sql);
    change_order_log($od_no, '입금대기');    
}
...
function change_order_status_7($od_no)
{
    ...
    if( $od['sellerpay_yes']==1 ){
        $sql = " update shop_order set dan = '7', sellerpay_yes = '2', return_date = '".TB_TIME_YMDHIS."', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
        $check=true;
    }else{
        $sql = " update shop_order set dan = '7', return_date = '".TB_TIME_YMDHIS."', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
    }
	sql_query($sql);
    ...
    change_order_log($od_no, '배송 후 반품');            
}
...
function change_order_status_8($od_no)
{
	$sql = " update shop_order
			  ...
			  where od_no = '$od_no' ";
	sql_query($sql);
    change_order_log($od_no, '배송 후 교환');    
}
...
function change_order_status_9($od_no)
{
    ...
    change_order_log($od_no, '배송 전 환불');        
}
...
function change_order_status_11($od_no, $delivery='', $delivery_no='')
{
    // 2023-05-15
    if( empty($delivery) ){
        $sql = " update shop_order set dan = '11', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
    }else{
        $sql = " update shop_order set dan = '11', delivery = '$delivery', delivery_no = '$delivery_no', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
    }
    sql_query($sql);
    change_order_log($od_no, '반품 중');                
}
...
function change_order_return_cancel($od_no)
{
    $sql = " update shop_order
              ...
              where od_no = '$od_no' ";
    sql_query($sql);
    change_order_log($od_no, '반품신청 철회');                    
}
// '교환신청' 상태로 변경
...
function change_order_status_13($od_no, $delivery='', $delivery_no='')
{
    // 2023-05-15
    if( empty($delivery) ){
        $sql = " update shop_order set dan = '13', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
    }else{
        $sql = " update shop_order set dan = '13', delivery = '$delivery', delivery_no = '$delivery_no', rcent_time = '".TB_TIME_YMDHIS."' where od_no = '$od_no' ";
    }
    sql_query($sql);
    change_order_log($od_no, '교환 중');                            
}
...
function change_order_change_cancel($od_no)
{
    $sql = " update shop_order
              ...
              where od_no = '$od_no' ";
    sql_query($sql);
    change_order_log($od_no, '교환신청 철회');                                
}

// 주문상태 변경 기록
function change_order_log($od_no, $memo)
{   
    unset($order_log);
    $order_log['od_no'] = $od_no ? $od_no : '';
    $order_log['mb_id'] = get_session('ss_mb_id') ? get_session('ss_mb_id') : '';
    $order_log['memo'] = $memo ? $memo : '';
...
// 상품 공통쿼리 - mwo simple 버전 
function sql_goods_list_extra($sql_search='')
{
	global $pt_id, $auth_good;
...
// 상품 검색쿼리 - mwo simple 버전 
function sql_goods_search_extra($sql_search='')
{
	global $pt_id, $auth_good;
...
// 상세 페이지출력 (배송/교환/반품)
function get_baesong_cont($mb_id)
{
	global $config, $sr;

	if($config['baesong_adm_cont_use']) {
		$str = TB_IS_MOBILE ? $config['baesong_cont2'] : $config['baesong_cont1'];
	} else {
		if($mb_id == 'admin')
			$str = TB_IS_MOBILE ? $config['baesong_cont2'] : $config['baesong_cont1'];
		else
			$str = TB_IS_MOBILE ? $sr['baesong_cont2'] : $sr['baesong_cont1'];
	}

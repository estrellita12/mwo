...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
	...
	for($k=2; $k<=$data->sheets[0]['numRows']; $k++)
	{
        ...
        // 주문정보
        $od = sql_fetch(" select dan from shop_order where od_id = '$od_id' and od_no = '$od_no' and seller_id='{$seller['seller_code']}' ");
        if(!$od) {
            $fail_count++;
            $fail_od_no[] = $od_no;
            continue;
		...
		$sql = " update shop_order
				    set delivery = '$delivery'
					  ...
					  , dan = '4'
				  where seller_id='{$seller['seller_code']}' and od_id = '$od_id'
				    and od_no = '$od_no' ";
		sql_query($sql);
		$succ_count++;
...
<div class="btn_confirm">
	<a href="./page.php?code=seller_odr_delivery" class="btn_large">확인</a>
</div>

...
$sql_search = " and seller_id = '{$seller['seller_code']}' ";

$sql = " select * from shop_order where od_id = '$od_id' {$sql_search} ";
$od = sql_fetch($sql);
if(!$od['od_id']) {
...
$amount = get_order_spay($od_id, $sql_search); // 결제정보 합계
...
<div id="sodr_pop" class="new_win">
	...
	<section id="anc_sodr_list">
		...
		<div class="local_desc02 local_desc">
			<p>
				...
				<a href="javascript:win_open('<?php echo TB_MYPAGE_URL; ?>/seller_odr_print.php?od_id=<?php echo $od_id; ?>','order_print','670','600','yes');" class="btn_small blue fr"><i class="fa fa-print"></i> 주문서출력</a>
			</p>
		</div>

		<form name="frmorderform" method="post" action="./seller_odr_statusupdate.php" onsubmit="return form_submit(this);">
		...
		<div class="tbl_head01">
			...
			<tr class="<?php echo $bg; ?>">
				...
				<td>
					...
					<a href="<?php echo TB_SHOP_URL; ?>/view.php?index_no=<?php echo $row['gs_id']; ?>" target="_blank"><?php echo get_it_image($row['gs_id'], $gs['simg1'], 40, 40); ?></a>
				</td>
				<td class="tal">
					<a href="<?php echo TB_MYPAGE_URL; ?>/page.php?code=seller_goods_form&w=u&gs_id=<?php echo $row['gs_id']; ?>" target="_blank"><?php echo get_text($gs['gname']); ?></a>
					<?php if($row['od_tax_flag'] && !$gs['notax']) echo '[비과세상품]'; ?>
					<?php echo $it_options; ?>
					<?php
					// 배송준비.배송중.배송완료
					$baesong_run = 0;
					if(in_array($row['dan'], array(3,4,5,10,11,12,13))) {
						$baesong_run++;
					?>
					<div class="frm_info">
                    ...
                    //2022-02-21 교환, 반품 관련 상태 선택 시 관리자 메모 입력 추가
                        $input_display = in_array($row['dan'], array(7,8,10,11,12,13)) ? 'block' : 'none';
                        echo "<textarea name='change_memo[{$i}]' class='change_memo_area' style='display:{$input_display}' placeholder='교환/반품에 대한 내용을 메모할 수 있습니다.'>{$row['change_memo']}</textarea>";                        
                    ?>                    
	...
	<section id="anc_sodr_pay" class="new_win_desc mart30">
		<h3 class="anc_tit">주문 상태 변경 내역
				<a href="javascript:win_open('<?php echo TB_MYPAGE_URL; ?>/seller_odr_log.php?od_id=<?php echo $od_id; ?>','order_log','670','400','yes');" class="btn_small red fr">+ CS일반</a>
</h3>
		<div class="tbl_head01">
			<table id="sodr_list">
			<colgroup>
			...
			<tbody class="list">
			<?php
			//$cssql = " select * from shop_order_log where od_no = '$od_no' order by update_time desc ";
			$cssql = " (select mb_id,'교환/반품 메모 수정' as memo,change_memo,change_date as update_time from shop_order_change_memo where od_no='$od_no' and change_memo != '' ) ";
            $cssql .= " union ";
            $cssql .= " (select mb_id,memo,'' as change_memo,update_time from shop_order_log where od_no='$od_no') ";
            $cssql .= " order by update_time desc ";
			$csresult = sql_query($cssql);
			for($i=1; $csrow=sql_fetch_array($csresult); $i++) {
            ?>
            <tr>
                <td><?php echo $i ?></td>
                <td><?php echo $csrow['mb_id'] ?></td>
                <td class="tal"><?php echo $csrow['memo'] ?></td>
                <!-- <td><?php echo $csrow['change_memo'] ?></td> -->
                <td><?php echo $csrow['update_time'] ?></td>
            </tr>
            <?php
            }
	        ?>
	...
	<section id="anc_sodr_pay" class="new_win_desc mart30">
		...
		<?php echo $pg_anchor; ?>
		<form name="frmorderreceiptform" action="./seller_odr_formupdate.php" method="post" autocomplete="off">
		<input type="hidden" name="od_id" value="<?php echo $od_id; ?>">
			...
			<section id="anc_sodr_paymo" class="compare_right">
				...
				<div class="tbl_frm01">
					...
					<tr>
						...
						<td><?php echo $od['od_mobile']?'모바일':'PC'; ?> 쇼핑몰에서 주문</td>
					</tr>
					<tr>
	...
	<section id="anc_sodr_memo">
		...
		<?php echo $pg_anchor; ?>
		<div class="local_desc02 local_desc">
			<p>현재 열람 중인 주문에 대한 내용을 메모하는곳입니다 (관리자만 수정 가능).</p>
		</div>

		<form name="frmorderform3" action="./seller_odr_formupdate.php" method="post">
		<input type="hidden" name="od_id" value="<?php echo $od_id; ?>">
		<input type="hidden" name="mod_type" value="memo">

		<label for="shop_memo" class="sound_only">관리자메모</label>
        <div class="frm_textbox"><?php echo nl2br($od['shop_memo']); ?></div>
		<div class="btn_confirm">
	...
	<section id="anc_sodr_addr">
		...
		<form name="frmorderform2" action="./seller_odr_formupdate.php" method="post">
		<input type="hidden" name="od_id" value="<?php echo $od_id; ?>">
...
function form_submit(f)
{
	...
	if(status == "운송장번호수정") {
		f.action = "./seller_odr_baesongupdate.php";
		return true;
	}

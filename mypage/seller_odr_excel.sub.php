...
for($i=2; $row=sql_fetch_array($result); $i++)
{
    ...
    $optsql = " select ct_option from shop_cart where od_id = '{$row['od_id']}' and gs_id = '{$gs['index_no']}' ";
	$optrow = sql_fetch($optsql,false);
    $optName = $optrow['ct_option'];
	...
	$excel->setActiveSheetIndex(0)
		...
		->setCellValueExplicit($char++.$i, $optName, PHPExcel_Cell_DataType::TYPE_STRING)

...
if($_POST['act_button'] == '결제완료') {
	$od = get_order($od_id);
	if($od['dan'] != 1)
		alert('입금대기 상태에서만 결제완료로 변경 가능합니다.');

	if(!in_array($od['paymethod'], array('무통장','가상계좌')))
		alert('무통장, 가상계좌만 결제완료로 변경 가능합니다.');
...
else if($_POST['act_button'] == "주문취소")
{
	$od = get_order($od_id);
	if($od['dan'] != 1)
		alert('입금대기 상태에서만 주문취소 가능합니다.');

	if(!in_array($od['paymethod'], array('무통장','가상계좌')))
		alert('무통장, 가상계좌만 주문취소 가능합니다.');
...
else if($_POST['act_button'] == "주문상태저장")
{
...
$url = TB_MYPAGE_URL."/partner_odr_form.php?od_id=$od_id";

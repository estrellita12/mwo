...
<div class="tbl_frm01">
		...
		<td>
			<select name="expire_date">
				...
				for($i=1; $i<=5; $i++) {
					...
					$pric = $i * (int)$pb['gb_term_price'];
					echo "<option value=\"{$date}^{$pric}\">{$date}개월 (".number_format($pric)."원)</option>\n";
				}
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			...
			<strong id="tot_term_price" class="fc_red"><?php echo number_format($pb['gb_term_price']); ?></strong>원
...
<script>
$(function(){
	$("select[name=expire_date]").change(function() {
		var info = $(this).val().split('^');
		var price = parseInt(info[1]);

		$("input[name=term_price]").val(price);
		$("#tot_term_price").empty().html(number_format(String(price)));
	});
});
</script>

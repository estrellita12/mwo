...
if(isset($q_state) && is_numeric($q_state)) {
	if($q_state)
		$sql_search .= " and (find_in_set('{$member['id']}', use_hide) >= 1) ";
	else
		$sql_search .= " and (find_in_set('{$member['id']}', use_hide) = 0) ";
}
...
$btn_frmline = <<<EOF
<input type="submit" name="act_button" value="선택상품감춤" class="btn_lsmall bx-white" onclick="document.pressed=this.value">
<input type="submit" name="act_button" value="선택상품노출" class="btn_lsmall bx-white" onclick="document.pressed=this.value">
EOF;

$colspan = 10;
$t_colspan = 3;
if($config['pf_payment_yes']) {
	$colspan++;
	$t_colspan++;
}
?>
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td colspan="3">
			...
			<input type="text" name="stx" value="<?php echo $stx; ?>" class="frm_input" size="40">
		</td>
	</tr>
	<tr>
...
<form name="fgoodslist" id="fgoodslist" method="post" action="./partner_goods_admlist_update.php" onsubmit="return fgoodslist_submit(this);">
...
<div class="tbl_head02">
	...
	<tr>
		...
		<th scope="col"><?php echo subject_sort_link('gname',$q2); ?>상품명</a></th>
		<th scope="col"><?php echo subject_sort_link('reg_time',$q2); ?>최초등록일</a></th>
		<th scope="col"><?php echo subject_sort_link('isopen',$q2); ?>진열</a></th>
		<th scope="col" colspan="<?php echo $t_colspan; ?>" class="th_bg">가격정보</th>
	</tr>
	<tr class="rows">
		<th>노출상태</th>
		<th>카테고리</th>
		<th><?php echo subject_sort_link('update_time',$q2); ?>최근수정일</a></th>
		<th><?php echo subject_sort_link('stock_qty',$q2); ?>재고</a></th>
		<th class="th_bg"><?php echo subject_sort_link('normal_price',$q2); ?>시중가</a></th>
		<th class="th_bg"><?php echo subject_sort_link('goods_price',$q2); ?>판매가</a></th>
		<th class="th_bg"><?php echo subject_sort_link('gpoint',$q2); ?>포인트</a></th>
		<?php if($config['pf_payment_yes']) { ?>

...
if($w == "") {
	$gs['mb_id']		= $seller['seller_code'];
	$gs['gcode']		= time();  // 2022-02-08
...
if($w == "u" && $bak) {
    $frm_submit .= PHP_EOL.'<a href="./page.php?code='.$bak.$qstr.'&page='.$page.'" class="btn_large bx-white">목록</a>';
	$frm_submit .= '<a href="./page.php?code=seller_goods_form" class="btn_large bx-red">추가</a>'.PHP_EOL;
}
$frm_submit .= '</div>';

if($member['grade'] == 9) $frm_submit="";
...
<div class="local_desc02 local_desc">
	<p>선택된 카테고리에 <span class="fc_084">최상위 카테고리는 대표 카테고리로 자동설정</span>되며, 1개의 카테고리를 반드시 등록하셔야 합니다.</p>
    <p class="fc_red">골프클럽,골프용품,골프패션을 제외한 카테고리는 현재 동작하지 않습니다.</p>
</div>
<div class="tbl_frm02">
	<table>
	<colgroup>
	...
	<tr>
		<th scope="row">선택된 카테고리<br><span class="fc_red">(1개만 등록가능)</span></th>
		<td>
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			<input type="text" name="mb_id" value="<?php echo $gs['mb_id']; ?>" id="mb_id" required itemname="업체코드" class="required frm_input" readonly style="background-color:#ddd;">
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<span class="fc_125 marl5">(사방넷 자체상품코드 : JS<?php echo $gs['gcode']; ?>, 사방넷 품번코드 : <?php echo $gs['sbcode'] ?>)</span>
		</td>
	</tr>
	<tr>
		<th scope="row">상품명</th>
	...
	<tr>
		<th scope="row">브랜드</th>
		<td><input type="text" name="brand_nm" value="<?php echo $gs['brand_nm']; ?>" class="frm_input"></td>
	</tr>
	...
	<tr>
		...
		<td><input type="text" name="make_year" value="<?php echo $gs['make_year']; ?>" class="frm_input" placeholder="2022" onInput="this.value=this.value.replace(/[^0-9]/g,'');"></td>
	</tr>
	<tr>
		<th scope="row">제조일자(8자리)</th>
		<td><input type="text" name="make_dm" value="<?php echo $gs['make_dm']; ?>" class="frm_input" placeholder="20220222" onInput="this.value=this.value.replace(/[^0-9]/g,'');"></td>
	</tr>
	<tr>
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<span class="fc_125 marl5">(사방넷 판매가)</span>
		</td>
	</tr>
	<tr>
		<th scope="row">포인트</th>
...
<div class="tbl_frm02">
    ...
    <tr>
        <th>동영상</th>
        <td>
            <a href="<?php echo TB_MYPAGE_URL; ?>/seller_goods_video_form.php?gs_id=<?php echo $gs_id; ?>" onclick="win_open(this,'seller_goods_video_form','900','600','yes');return false;" class="btn_small bx-white">업로드 바로가기</button>
        </td>
    </tr>
    -->
    <?php } ?>
...
function fregform_submit(f) {
    ...
    //2023-04-17 옵션정보 없을 경우 update 페이지 넘어가기 전 차단 (넘어갔다 돌아올 시 기입력정보 사라지기 때문에 불편하다는 요청사항에 따른 변경) 
    var options = new Array();
    $("input[name='opt_id[]']").each(function() {
        new_opt = $(this).val();
        if(new_opt == "") return true;

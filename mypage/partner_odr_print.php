...
<div id="sodr_print_pop" class="new_win">
	...
	{
		$amount = get_order_spay($row['od_id'], " and seller_id = '{$member['id']}' "); // 주문별 합계
	...
	<div class="sodr_print_pop_list">
		...
		<div class="tbl_head01">
			...
			$sql2 = " select * from shop_order where od_id = '{$row['od_id']}' order by od_time desc, index_no asc ";
			$res2 = sql_query($sql2);
			while($row2=sql_fetch_array($res2)) {
				$gs = unserialize($row2['od_goods']);

				// 자신의 상품주문건만 노출한다.
				if($row2['seller_id'] != $member['id'])
					continue;
			...
			<tr>
				...
				<td class="tac"><?php echo $gw_status[$row2['dan']]; ?></td>
			</tr>
			<?php
			}
			?>

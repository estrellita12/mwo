...
$excel->setActiveSheetIndex(0)
	...
	->setCellValue($char++.'1', '교환신청일시')
	->setCellValue($char++.'1', '교환완료일시')
	->setCellValue($char++.'1', '반품신청일시')
	->setCellValue($char++.'1', '반품완료일시')
	->setCellValue($char++.'1', '환불일시')
	...
	->setCellValue($char++.'1', '반품/교환 사유')
	->setCellValue($char++.'1', '기록');
...
for($i=2; $row=sql_fetch_array($result); $i++)
{
    ...
    $cssql = "SELECT group_concat(memo,'/',mb_id,'/',update_time) as memo FROM shop_order_log WHERE od_no='{$row['od_no']}'  order by update_time desc";
    $csrow = sql_fetch($cssql,false);
    $memo = $csrow['memo'];
	...
	$excel->setActiveSheetIndex(0)
		...
		->setCellValueExplicit($char++.$i, $row['od_pwd'].$sodr['od_test'], PHPExcel_Cell_DataType::TYPE_STRING)
		...
		->setCellValueExplicit($char++.$i, $row['change_date2'], PHPExcel_Cell_DataType::TYPE_STRING)
		->setCellValueExplicit($char++.$i, $row['change_date'], PHPExcel_Cell_DataType::TYPE_STRING)
		->setCellValueExplicit($char++.$i, $row['return_date2'], PHPExcel_Cell_DataType::TYPE_STRING)
		->setCellValueExplicit($char++.$i, $row['return_date'], PHPExcel_Cell_DataType::TYPE_STRING)
		->setCellValueExplicit($char++.$i, $row['refund_date'], PHPExcel_Cell_DataType::TYPE_STRING)
		...
		->setCellValueExplicit($char++.$i, $memo, PHPExcel_Cell_DataType::TYPE_STRING);
}

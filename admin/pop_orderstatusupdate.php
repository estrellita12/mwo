...
for($i=0; $i<$chk_count; $i++)
{
	...
	switch($change_status) {
		...
		case '4': // 배송중
			//if($current_status != 3) continue;
			if($current_status == 4) continue;
			change_order_status_4($od_no, $delivery, $delivery_no);
			$od_sms_baesong_check++;
			break;
		case '5': // 배송완료
			//if(!in_array($current_status, array(3,4,10,12))) continue;
		...
		case '10': // 반품신청
			//if($current_status != 5 ) continue;
			if($current_status == 10) continue;
			change_order_status_10($od_no, $od_id);
			break;
		case '11': // 반품중
			//if(!in_array($current_status, array(5,10))) continue;
			if($current_status == 11) continue;
			change_order_status_11($od_no, $delivery, $delivery_no);
			break;
		case '12': // 교환신청
			//if($current_status != 5 ) continue;
			if($current_status == 12) continue;
			change_order_status_12($od_no, $od_id);
			break;
		case '13': // 교환중
			//if(!in_array($current_status, array(5,12))) continue;
			if($current_status == 13) continue;
			change_order_status_13($od_no, $delivery, $delivery_no);
			break;
	}
    update('shop_order',array('change_memo' => $_POST['change_memo'][$k]),'WHERE od_no="'.$od_no.'"');
    insert('shop_order_change_memo',array('change_memo' => $_POST['change_memo'][$k],"od_no"=>$od_no,"current_dan"=>$current_status,"change_dan"=>$change_status,"mb_id"=>$member['id'],"access_ip"=>$_SERVER['REMOTE_ADDR'],"change_date"=>TB_TIME_YMDHIS));
...
if($od_sms_ipgum_check)
	icode_order_sms_send($pt_id, $od_hp, $od_id, 3); // 결제완료 문자
			...
			if($od['od_tno'] && ($od['paymethod'] == '신용카드' || $od['paymethod'] == '간편결제' || $od['paymethod'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && $od['paymethod'] == '삼성페이')) {
				...
				switch($od['od_pg']) {
					...
					case 'nicepay':
						...
						$_REQUEST['TID']				= $od['od_tno'];
						$_REQUEST['CancelAmt']			= $od_receipt_price;
						$_REQUEST['CancelMsg']			= '쇼핑몰 운영자 승인 취소';
						...
						*/
						...
						$res_cd  = trim($responseDTO->getParameter("ResultCode")); // 결과코드 (취소성공: 2001, 취소성공(LGU 계좌이체):2211)
						$res_msg = trim($responseDTO->getParameterUTF("ResultMsg")); // 결과메시지

						if( !in_array($res_cd, array('2001','2211')) ) {
							$pg_res_cd = $res_cd;
							$pg_res_msg = $res_msg;
						}
						break;

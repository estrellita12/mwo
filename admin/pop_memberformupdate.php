...
if($_POST['anew_date']) $mfrm['anew_date'] = $_POST['anew_date']; // 가맹점등업일
if($_POST['term_date']) $mfrm['term_date'] = $_POST['term_date']; // 가맹점만료일
if($_POST['passwd'])	$mfrm['passwd']    = $_POST['passwd']; // 패스워드
...
if($pt['mb_id']) {
	$sql = " update shop_partner
				set bank_name		= '{$_POST['bank_name']}',
					...
					shop_intro_yes	= '{$_POST['shop_intro_yes']}',
					cert_admin_yes	= '{$_POST['cert_admin_yes']}'
			  where mb_id = '$mb_id'";
	sql_query($sql, FALSE);
}

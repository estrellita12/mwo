	...
	if($coupon['cp_use_goods']) {
		...
		foreach($arr_use_goods as $gs_id){
			...
			if($gs['index_no']) {
				...
				$ck_use_goods .= "<a href=\"".TB_SHOP_URL."/view.php?index_no={$gs_id}\" target=\"_blank\"><img src=\"{$gs_img}\" class=\"pr_img\"></a>";
				$ck_use_goods .= "<p>{$gs_name}</p><p class=\"mart5 bold\">{$gs_price}원</p>";
				$ck_use_goods .= "<a href=\"javascript:chk_del('{$gs_id}');\" class=\"bt_del\"><img src=\"".TB_ADMIN_URL."/img/bt_delete.gif\"></a>";
				$ck_use_goods .= "</li>";
	...
	<div class="tbl_frm01">
		...
		<tr id="ids_age">
			...
			<td>
				...
				<span class="fc_197 marl5">생년월일 앞 4자리만 입력 (Ex : <?php echo TB_TIME_YEAR-5; ?>년생부터 <?php echo TB_TIME_YEAR; ?>년생까지)</span>
			</td>
		</tr>
		<tr id="ids_day">
		...
		<tr id="ids_gpart">
			...
			<td>
				...
				<p>
					...
					<a href="<?php echo TB_ADMIN_URL; ?>/goods/goods_coupon_goods.php" onclick="win_open(this,'cp_goods','850','800','yes'); return false;" class="btn_small red marl10">상품등록</a>
				</p>

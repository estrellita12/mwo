...
if($w == "") {
    ...
    $presql = " SELECT * FROM shop_goods WHERE index_no='$gs_id' ";
    $prerow = sql_fetch($presql);
/*
    if ( $value['isopen'] != $prerow['isopen'] ) {
        $sbflag = true;
    }
*/
    $logmsg = "";
    $logres = array("mb_id"=>"판매자ID","gname"=>"상품명","opt_subject"=>"상품 주문 옵션", "normal_goods"=>"시중가격","goods_price"=>"판매가격","supply_price"=>"공급가격","isopen"=>"판매여부");
    foreach($logres as $col=>$comment){
        if( $prerow[$col] != $value[$col] ){
            if( !empty($msg) ) $msg .= ",";
            $msg .= " [{$comment}] {$prerow[$col]} => {$value[$col]} ";
        }
    }
    $logsql = "insert into shop_goods_log set gs_id='{$gs_id}', mb_id='".get_session('ss_mb_id')."', update_time='".TB_TIME_YMDHIS."', memo='{$msg}' ";
    sql_query($logsql);

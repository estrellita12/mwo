...
<div class="local_frm01">
    ...
    <a href="#" id="goodsSelectedExcel" class="btn_lsmall white"><i class="fa fa-file-excel-o"></i> 선택 엑셀저장</a><!--2022-11-25 선택 엑셀 다운로드 추가-->
</div>
<div class="tbl_head02">
	...
	<tr class="rows">
		...
		<th scope="col" class="th_bg"><?php echo subject_sort_link('goods_price',$q2); ?>판매가(C)</a></th>
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <td rowspan="2" class="tar"><?php echo round( (($row['goods_price']-$row['supply_price'])/$row['goods_price'])*100 ) ?>%</td>
...
// 엑셀저장 선택값 저장
$("#goodsSelectedExcel").on("click", function() {
    var s_gs_chk = new Array();
    var s_gs_id = "";
    var $s_el_chk = $("input[name='chk[]']");
    
    $s_el_chk.each(function(index) {
        if($(this).is(":checked")) {
            s_gs_chk.push($("input[name='gs_id["+index+"]']").val());
        }
    });

    if(s_gs_chk.length > 0) {
        s_gs_id = s_gs_chk.join();
    }
    ...
    if(s_gs_id == "") {
        ...
        this.href = "./goods/goods_list_selected_excel.php?s_gs_id="+s_gs_id;
        return true;
    }
});	

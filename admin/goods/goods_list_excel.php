...
for($i=2; $row=sql_fetch_array($result); $i++)
{
    ...
    $seller_sql = " select mb_id from shop_seller where seller_code = '{$row['mb_id']}' ";
    $seller_row = sql_fetch($seller_sql);
    $seller_id = $seller_row['mb_id'];
	...
	->setCellValueExplicit($char++.$i, $seller_id, PHPExcel_Cell_DataType::TYPE_STRING)

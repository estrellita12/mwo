...
if(isset($q_seller) && $q_seller)
	$sql_search .= " and b.mb_id = '$q_seller' ";
...
<a href="#" id="frmOptionsExcel" class="btn_lsmall white"><i class="fa fa-file-excel-o"></i> 엑셀저장</a>
EOF;
?>
...
<div class="local_ov mart30">
	...
	<span class="ov_a">
		<select id="page_rows" onchange="location='<?php echo "{$_SERVER['SCRIPT_NAME']}?{$q1}&page=1"; ?>&page_rows='+this.value;">
            ...
            <?php echo option_selected('1000', $page_rows, '1000줄 정렬'); ?>
		</select>
	</span>
</div>
...
<div class="tbl_head02">
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td rowspan="2"><a href="<?php echo TB_SHOP_URL; ?>/view.php?index_no=<?php echo $row['gs_id']; ?>" target="_blank"><?php echo get_it_image($row['gs_id'], $row['simg1'], 40, 40); ?></a></td>
...
// 엑셀저장 선택값 저장
$("#frmOptionsExcel").on("click", function() {
    //var type = $(this).attr("id");
    var io_chk = new Array();
    var io_id = "";
    var $el_chk = $("input[name='chk[]']");
    
    $el_chk.each(function(index) {
        if($(this).is(":checked")) {
            io_chk.push($("input[name='io_no["+index+"]']").val());
        }
    });
    ...
    if(io_id == "") {
        ...
        this.href = "./goods/goods_list_excel_option.php?io_id="+io_id;
        return true;
    }
});		

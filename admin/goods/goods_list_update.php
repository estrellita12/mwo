...
if($_POST['act_button'] == "선택삭제")
{
	...
	for($i=0; $i<$count; $i++)
	{
		...
		include(TB_ADMIN_PATH.'/goods/goods_delete.inc.php');
	}
}
else if($_POST['act_button'] == "선택순위수정")
{
	for($i=0; $i<$count; $i++)
	{
		// 실제 번호를 넘김
...
else if($_POST['act_button'] == "선택상품복사")
{
	...
	for($i=0; $i<$count; $i++)
	{
		...
		// include 전에 $gs_id 값을 반드시 넘겨야 함
		$gs_id = trim($_POST['gs_id'][$k]);
		include(TB_ADMIN_PATH.'/goods/goods_copy.inc.php');
	}
}
...
else if($_POST['act_button'] == "선택승인")
{
	for($i=0; $i<$count; $i++)
	{
		...
		$gs_id = trim($_POST['gs_id'][$k]);
        sb_goods_update( $gs_id, $_SESSION['ss_mb_id'] ); // 2022-08-02

		$sql = " update shop_goods set shop_state = '0', state_msg='' where index_no = '$gs_id' ";
		sql_query($sql);

	}
}
else if($_POST['act_button'] == "선택보류")
{
	for($i=0; $i<$count; $i++)
	{
		// 실제 번호를 넘김
		...
		$sql = " update shop_goods set shop_state = '2' where index_no = '$gs_id' ";
		sql_query($sql);
	}
}
else if($_POST['act_button'] == "선택대기")
{
	for($i=0; $i<$count; $i++)
	{
		// 실제 번호를 넘김
		...
		$sql = " update shop_goods set shop_state = '1' where index_no = '$gs_id' ";
		sql_query($sql);
	}
} else {
	alert();

...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
	...
	for($i=4; $i<=$data->sheets[0]['numRows']; $i++)
	{
		...
		$make_year		= addslashes(trim($data->sheets[0]['cells'][$i][$j++])); // 생산연도
		$make_dm		= addslashes(trim($data->sheets[0]['cells'][$i][$j++])); // 제조일자
		...
		$value['make_year']		= $make_year; // 생산연도
		$value['make_dm']		= $make_dm; // 제조일자
        ...
        $presql = " SELECT * FROM shop_goods WHERE gcode='$gcode' ";
        $prerow = sql_fetch($presql);
        $logmsg = "";
        $logres = array("gname"=>"상품명", "normal_goods"=>"시중가격","goods_price"=>"판매가격","supply_price"=>"공급가격","isopen"=>"판매여부");
        foreach($logres as $col=>$comment){
            if( $prerow[$col] != $value[$col] ){
                if( !empty($logmsg) ) $logmsg .= ",";
                $logmsg .= " [{$comment}] {$prerow[$col]} => {$value[$col]} ";
            }
        }
        $logsql = "insert into shop_goods_log set gs_id='{$prerow['index_no']}', mb_id='".get_session('ss_mb_id')."', update_time='".TB_TIME_YMDHIS."', memo='{$logmsg}' ";
        sql_query($logsql);
...
<div class="btn_confirm">
	<a href="<?php echo TB_ADMIN_URL; ?>/goods.php?code=xls_mod" class="btn_large">확인</a>
</div>

...
if(isset($q_stt_msg))       $qstr .= "&q_stt_msg=$q_stt_msg";
...
$sql_search = " where left(mb_id,3)='AP-' and shop_state = '1' ";
...
if($stx) {
    switch($sfl) {
		...
		case "maker" :
		case "origin" :
        case "gcode" : //2022-12-01 다중검색 기능 추가
            if(!empty($stx)){
                $stx_nonspace = preg_replace("/\s+/", "", $stx);
                $stx_multi = str_replace(",", "|", $stx_nonspace);
                $sql_search .= " and $sfl REGEXP '$stx_multi' ";
            }
            break;             
		case "model" :
...
if($q_stt_msg) {
    if( $q_stt_msg == 1 ){
        $sql_search .= " and ( state_msg like '%공급가%' or state_msg like '%판매가%' ) ";
    }else if($q_stt_msg == 2){
        $sql_search .= " and ( state_msg like '%상품%' or state_msg like '%진열%' ) ";
    }
}
...
<input type="submit" name="act_button" value="선택보류" class="btn_lsmall bx-white" onclick="document.pressed=this.value">
<a href="./goods/goods_supply_excel.php?$q1" class="btn_lsmall bx-white"><i class="fa fa-file-excel-o"></i> 엑셀저장</a>
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td colspan="3">
			<select name="sfl">
				...
				<?php echo option_selected('sbcode', $sfl, '사방넷품번코드'); ?>
    ...
    <tr>
        ...
        <td>
            <?php echo radio_checked('q_stt_msg', $q_stt_msg,  '', '전체'); ?>
            <?php echo radio_checked('q_stt_msg', $q_stt_msg, '1', '가격 수정'); ?>
            <?php echo radio_checked('q_stt_msg', $q_stt_msg, '2', '기타 수정 '); ?>
        </td>
    </tr>
	</tbody>
	</table>
...
<div class="tbl_head02">
	...
	<tr class="rows">
		...
		<th scope="col" class="th_bg"><?php echo subject_sort_link('normal_price',$q2); ?>소비자가</a></th>
		<th scope="col" class="th_bg"><?php echo subject_sort_link('supply_price',$q2); ?>원가(공급가)</a></th>
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td rowspan="3"><?php echo $num--; ?></td>
		<td rowspan="3"><a href="<?php echo TB_SHOP_URL; ?>/view.php?index_no=<?php echo $gs_id; ?>" target="_blank"><?php echo get_it_image($gs_id, $row['simg1'], 40, 40); ?></a></td>
		<td><?php echo $row['gcode']; ?>(<?php echo $row['index_no']; ?>)</td>
        <td colspan="2" class="tal">
            <a target="_blank" href="https://search.shopping.naver.com/search/all?frm=NVSHATC&iq=<?php echo $row[model] ?>&query=<?php echo get_text($row['gname']); ?>&sort=price_asc"><?php echo get_text($row['gname']); ?></a>
        </td>
		<td><?php echo substr($row['reg_time'],2,8); ?></td>
		<td><?php echo $gw_isopen[$row['isopen']]; ?></td>
		<td rowspan="3" class="tar"><?php echo number_format($row['normal_price']); ?></td>
		<td rowspan="3" class="tar"><?php echo number_format($row['supply_price']); ?></td>
		<td rowspan="3" class="tar"><?php echo number_format($row['goods_price']); ?></td>
		<td rowspan="3" class="tar"><?php echo round( (($row['goods_price']-$row['supply_price'])/$row['goods_price'])*100 ) ?>%</td>
		<td rowspan="3"><a href="./goods.php?code=form&w=u&gs_id=<?php echo $gs_id.$qstr; ?>&page=<?php echo $page; ?>&bak=<?php echo $code; ?>" class="btn_small">수정</a></td>
	</tr>
	<tr class="<?php echo $bg; ?>">
		<td class="fc_00f"><?php echo $row['sbcode']; ?></td>
	...
	<tr class="<?php echo $bg; ?>">
		<td colspan="5" class="tal fs12"><?php echo $row['state_msg']; ?></td>
	</tr>

...
// 가맹점등업일
if(is_null_time($mb['anew_date']))
	$mb['anew_date'] = '';

// 가맹점만료일
if(is_null_time($mb['term_date']))
	$mb['term_date'] = '';
	...
	<div class="tbl_frm01">
		...
		<tr>
			...
			<td>
				...
				<button type="button" class="btn_small red" onclick="member_leave();">회원탈퇴</button>
			</td>
			<th scope="row">아이디</th>
		...
		<?php if(is_partner($mb['id'])) { // 가맹점인가? ?>
		<tr class="pt_pay_fld">
			<th scope="row" class="th_bg fc_00f">가맹점등업일</th>
			<td><input type="text" name="anew_date" value="<?php echo $mb['anew_date'];?>" id="anew_date" class="frm_input" size="12" maxlength="10"></td>
			<th scope="row" class="th_bg fc_00f">가맹점만료일</th>
			<td><input type="text" name="term_date" value="<?php echo $mb['term_date'];?>" id="term_date" class="frm_input" size="12" maxlength="10"></td>
		</tr>
		<?php } ?>
		<tr class="pt_pay_fld">
		...
		<tr class="pt_pay_fld">
			...
			<th scope="row" class="th_bg fc_00f">폐쇄몰설정</th>
			<td>
				<label><input type="checkbox" name="shop_intro_yes" value="1"<?php echo $pt['shop_intro_yes']?' checked':''; ?>> 폐쇄몰 사용</label>
				<label><input type="checkbox" name="cert_admin_yes" value="1"<?php echo $pt['cert_admin_yes']?' checked':''; ?>> 관리자 인증 후 회원가입</label>
			</td>
		</tr>
		<tr class="pt_pay_fld">
...
$(function() {
	...
	$("#anew_date, #term_date").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true, yearRange: "c-99:c+99" });
});
</script>

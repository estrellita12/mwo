...
// 배너코드를 얻는다.
$position = $gw_pbanner[$super['theme']];
if(!count($position)) {
	echo <<<EOF
	<div class="local_desc01 local_desc">
		<p class="lh6">
			{$super['theme']} 스킨의 노출위치 변수가 설정되어있지 않습니다.<br>이 경우는 별도 디자인작업을 통한 변수 설정 누락건이며 담당 디자이너 및 개발자에게 문의하시기 바랍니다.<br>변수 설정파일 경로 : <b>/extend/shop.extend.php</b> 에서 설정 이후 이용 하시기 바랍니다.
		</p>
	</div>
EOF;
	return;
...
$frm_submit = '<div class="btn_confirm">
	...
	<a href="./design.php?code=banner_list'.$qstr.'&page='.$page.'" class="btn_large bx-white">목록</a>'.PHP_EOL;
if($w == 'u')
...
<form name="fbanner" id="fbanner" action="./design/banner_form_update.php" method="post" enctype="MULTIPART/FORM-DATA">

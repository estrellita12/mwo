...
$sql = "select id from shop_member where homepage = '{$_SERVER['HTTP_HOST']}' and homepage <> '' ";
$row = sql_fetch($sql);
if($row['id']) {
	$temp_shopid = $row['id'].'.';
} else {
...
if(!$is_member && $config['shop_intro_yes']) {
	...
	if(preg_match("/login_check2.php/", $_SERVER['PHP_SELF'])) $intro_run++;
	...
	if(preg_match("/seller_login.php/", $_SERVER['PHP_SELF'])) $intro_run++;
	if(preg_match("/seller_reg.php/", $_SERVER['PHP_SELF'])) $intro_run++;
	if(preg_match("/seller_reg_from.php/", $_SERVER['PHP_SELF'])) $intro_run++;
	if(preg_match("/seller_reg_from_update.php/", $_SERVER['PHP_SELF'])) $intro_run++;

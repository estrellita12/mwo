...
<h2 class="pg_tit subpg_tit">주문결제</h2>
...
<div class="tbl_wrap_box">
	<p class="pg_cnt marb15">
		<strong class="line">주문상품</strong>주문하실 상품 내역에 수량 및 주문금액이 틀리지 않는지 반드시 확인하시기 바랍니다.
...
<section id="sod_fin_orderer" >
	...
	<div class="tbl_frm01 tbl_wrap tbl_wrap1">
        ...
        <!-- 현대리바트 start -->
          ...
          <?php  if($is_member && $pt_id == 'golf') { ?>	  
     
	    <!-- 리바트에 강제적으로 넣어준거임. -->
	<input type="hidden" name="use_point" value="0">
			
	<!-- 기본금 현금영수증 신청 -->
    <section id="tax_section2"> 
		<tr>
		...
		<tr id="taxsave_fld_1" style="display:none">
			...
			<td>
				<label>
					<input type="text" name="tax_hp" class="frm_input" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13">
				</label>
				<span class="frm_info">
		...
		<tr id="taxsave_fld_2" style="display:none">
			...
			<td><label><input type="text" name="tax_saupja_no" class="frm_input" size="20"></label></td>
		</tr>
        
		<tr>
			<th scope="row">총 결제금액</th>
			<td>
				<input type="text" name="tot_price2" value="<?php echo number_format($tot_price); ?>" class="frm_input" size="12" readonly style="font-weight:bold;color:#ec0e03;"> 원
			</td>
		</tr>
		<!--20191210 에스크로 관련 약관 추가-->
		<tr>
			<td class="ec_service">
			<p style="text-align:center">※ 전자상거래 구매 안전 서비스 <br>안내</p>
			</td>
			<td>
				<p>전자금융거래법에 따라 금융감독(원) 위원회에 결제대금 예치업을 등록하였으며, 안전거래를 위해 구매 금액, 결제수단에 상관없이 모든 거래에 대하여 저희 쇼핑몰에서 가입한 구매안전서비스(에스크로)를 자동으로 적용하고 있습니다.</p>
			</td>
		</tr>
	...
	<?php } ?>
		...
		<tr>
			...
			<td>
				<label><input type="password" name="od_pwd" required itemname="비밀번호" class="frm_input required" size="20"></label>
		...
		<tr>
			...
			<td><input type="text" name="name" value="<?php echo $_COOKIE['gp_name'];?>" required itemname="이름" class="frm_input required" size="20"></td>
			<?php }else if($pt_id == 'golfu') { ?>
			<td><input type="text" name="name" value="<?php echo $_COOKIE['UserName'];?>" required itemname="이름" class="frm_input required" size="20"></td>
			<?php }else{ ?> 
			<td><label><input type="text" name="name" value="<?php echo $member['name']; ?>" required itemname="이름" class="frm_input required" size="20"></label></td>
			<?php } ?>
		</tr>
<!-- 		<tr> -->
<!-- 			<th scope="row">전화번호</th> -->
<!-- 			<td><label><input type="text" name="telephone" value="<?php echo $member['telephone']; ?>" class="frm_input" size="20"></label></td> -->
<!-- 		</tr> -->
		<tr>
			<th scope="row">휴대폰</th>
			<?php if($pt_id == 'golfpang'){  //골팡?>
			<td><input type="text" name="cellphone" value="<?php echo $_COOKIE['gp_phone']; ?>" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></td>
			<?php }else if($pt_id == 'golfu'){  //골프유닷넷?>
			<td><input type="text" name="cellphone" value="<?php echo $_COOKIE['UserMobile']; ?>" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></td>
			<?php }else{ ?> 
            <td><label><input type="text" name="cellphone" value="<?php echo $member['cellphone']; ?>" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></label></td>
			<?php } ?>
		...
		<tr>
			...
			<td>
				<div>
					<label><input type="text" readonly name="zip" value="<?php if($member['zip'] != null) { echo $member['zip']; } else { echo $address['b_zip']; }?>" required itemname="우편번호" class="frm_input required line_none" maxLength="5" size="8"></label><a href="javascript:win_zip('buyform', 'zip', 'addr1', 'addr2', 'addr3', 'addr_jibeon');" class="btn_small grey">주소검색</a>
				</div>
				<div class="padt8">
					<label><input type="text" readonly name="addr1" value="<?php if($member['addr1'] != null) { echo $member['addr1']; } else { echo $address['b_addr1']; } ?>" required itemname="주소" class="frm_input required line_none" size="60" readonly> 기본주소</label>
				</div>
				<div class="padt8">
					<label><input type="text" name="addr2" value="<?php if($member['addr2'] != null) { echo $member['addr2']; } else { echo $address['b_addr2']; } ?>" class="frm_input" size="60"> 상세주소</label>
				</div>
				<div class="padt8">
					<label><input type="text" name="addr3" value="<?php if($member['addr3'] != null) { echo $member['addr3']; } else { echo $address['b_addr3']; } ?>" class="frm_input" size="60" readonly> 참고항목</label>
		...
		<tr>
			<th scope="row">이메일</th>
			<td>
<!-- 	20200811이메일 셀렉추가로 폼변경	<label><input type="text" name="email" value="<?php echo $member['email']; ?>" email itemname="E-mail" required class="frm_input required" size="30"></label> -->
...
<section id="sod_fin_receiver">
	...
	<div class="tbl_frm01 tbl_wrap tbl_wrap1">
		...
		<tr>
			...
			<td><label><input type="text" name="b_name" required itemname="이름" class="frm_input required" size="20"></label></td>
		</tr>
<!-- 		<tr> -->
<!-- 			<th scope="row">전화번호</th> -->
<!-- 			<td><label><input type="text" name="b_telephone" class="frm_input" size="20"></label></td> -->
<!-- 		</tr> -->
		<tr>
			<th scope="row">휴대폰</th>
			<td><label><input type="text" name="b_cellphone" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></label></td>
		</tr>
		<tr>
			<th scope="row">주소</th>
			<td>
...
<section id="sod_fin_pay">
	...
	<div class="tbl_frm01 tbl_wrap tbl_wrap1">
		...
		<tr style="display:none;" class="bank_acc_tr">
			...
			<td><label><input type="text" name="deposit_name" value="<?php echo $member['name']; ?>" class="frm_input" size="12"></label></td>
		</tr>
...
<?php if($pt_id !='golf') { ?>
		...
		<tr class="taxbill_fld">
			...
			<td><label><input type="text" name="company_saupja_no" size="20" class="frm_input"></label></td>
		</tr>
		<tr class="taxbill_fld">
			<th scope="row">상호(법인명)</th>
			<td><label><input type="text" name="company_name" class="frm_input" size="20"> 예 : <?php echo $config['company_name']; ?></label></td>
		</tr>
		<tr class="taxbill_fld">
			<th scope="row">대표자</th>
			<td><label><input type="text" name="company_owner" class="frm_input" size="20"> 예 : 홍길동</label></td>
		</tr>
		<tr class="taxbill_fld">
			<th scope="row">사업장주소</th>
			<td><label><input type="text" name="company_addr" class="frm_input" size="60"></label></td>
		</tr>
		<tr class="taxbill_fld">
			<th scope="row">업태</th>
			<td><label><input type="text" name="company_item" class="frm_input" size="20"> 예 : 도소매</label></td>
		</tr>
		<tr class="taxbill_fld">
			<th scope="row">종목</th>
			<td><label><input type="text" name="company_service" class="frm_input" size="20"> 예 : 전자부품</label></td>
		</tr>
...
<!-- 20200521 비회원일때만 원래 나왔었는데 회원일 경우에도 나오도록 오류 요청 건 -->
...
<section id="guest_privacy">
	<h3 class="pg_cnt marb15">
			<strong class="<?=$line?>">개인정보 수집 및 이용</strong><?php if(!$is_member) { ?> 비회원으로 주문 시 포인트적립 및 추가 혜택을 받을 수 없습니다. <?}?>
	</h3>
	<div class="tbl_head02 tbl_wrap">
		<table>
...
function fbuyform_submit(f) {
	...
	var temp_point	= parseInt(no_comma(f.use_point.value));//포인트 사용금액
...
?>
    ...
    //현대복지몰(추후 사이트내 포인트 같이 사용시 적용처리가 필요할것으로 예상: use_point+use_point2)
...
?>
	...
	if(temp_point2 > mb_point2) {
		...
		$("#h_point").html(number_format(0) + "원");
		$("#tot_price3").html(f.tot_price.value + "원"); 
		return false;
	}

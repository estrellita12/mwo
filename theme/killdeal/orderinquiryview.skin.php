...
<div id="con_lf">
					...
					<tr>
						...
						<td class="vat tal">
							...
							<p class="padt3">
								...
								<?php } ?>
								<a href="<?php echo TB_SHOP_URL; ?>/view_user_form.php?gs_id=<?php echo $rw['gs_id']; ?>&od_id=<?php echo $rw['od_id']; ?>" onclick="win_open(this, 'winorderreview', '700', '600','yes');return false;" class="btn_ssmall bx-white">리뷰작성</a>
							</p>
							<?php } ?>
						</td>
					</tr>
	...
	<section id="sod_fin_pay">
		...
		<div class="tbl_frm01 tbl_wrap">
			...
			<tr>
				...
				<?php
				   if($pt_id =='golf'){
				       echo display_price($stotal['useprice']+$stotal['usepoint2']);
	               }else {

                       echo display_price($stotal['useprice']);
				   }
			 ?>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					...
					if($od['paymethod'] == '신용카드' || $od['paymethod'] == '복지카드')
					{
						if($od['od_pg'] == 'lg') {
							require_once TB_SHOP_PATH.'/settle_lg.inc.php';
							$LGD_TID      = $od['od_tno'];
	...
	<section id="sod_fin_cancel">
		...
		<div id="sod_fin_cancelfrm">
			...
			<!-- 20200221 기존 취소 사유 주석처리 select 변경
			<input type="text" name="cancel_memo" id="cancel_memo" required class="frm_input required" size="40" maxlength="100">
			-->
			<select name="cancel_memo" id="cancel_memo" required class="frm_input required">
                <option value="">취소사유를 선택해 주세요.</option>
                <option value="구매의사 없음">구매의사 없음</option>
                <option value="배송이지연되고 있음">배송이지연되고 있음</option>
                <option value="상품옵션 선택 잘못함">상품옵션 선택 잘못함</option>
            </select>
...
/*
echo("현대리바트 인자 출력하기\n");
echo("https://giftdev.e-hyundai.com/hb2efront_new/pointOpenAPIMagam.do");
echo("?ORDER_NO=".$order_no2);
echo("&ITEM_CD=".$item_cd2);
echo("&ORDER_GB=".$order_gb2);
echo("&ORDER_DM=".$order_dm2);
echo("&SHOP_NO=".$shop_no);
echo("&SHOPEVENT_NO=".$shopevent_no2);
echo("&MEM_NO=".$mem_no2);
echo("&TAX_GB=".$tax_gb2);
echo("&SALEPRICE=".$saleprice2);
echo("&POINT_AMT=".$point_amt2);
echo("&ETC_AMT=".$etc_amt2);
echo("&MEDIA_CD=MW");
echo("&DELI_AMT=".$deli_amt2);
echo("&ITEM_NM=".$item_nm2);
echo("&ITEM_PRICE=".$item_price2);
echo("&ORDER_QTY=".$order_qty2);
echo("&DC_PRICE=".$dc_price2);
//echo("&CTGR_L=".$ctgr_l2);
echo("&MAGAM_GB=".$magam_gb2);
echo("&MDA_GB=".$mda_gb2);
*/
?>
<script>

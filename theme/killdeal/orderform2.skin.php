...
if($pt_id=='dodamgolf' || $pt_id == 'golfu')
{
...
$sql = " select * from shop_cart where index_no IN ({$ss_cart_id}) and ct_select = '0' group by gs_id order by index_no ";
...
for($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    if($is_member) {
        ...
        if($pt_id == 'golfu')
        {
            //$point = floor($sum['price'] * 0.03);//포인트적립출력_20191226
            $point = $sum['point'];
        }
        else
        {
...
$tot_price = (int)$tot_sell_price + (int)$tot_send_cost; // 결제예정금액
...
if($pt_id == 'golfu'){ 
    ...
    //포인트사용액 자동출력_20200214
    $tot_price10_ = floor($tot_price/10); //소수점 버림
    $tot_price10 = floor($tot_price10_/100) * 100;//포인트 100원단위로 재설정

    //**쿠폰사용시 구매금액10% 금액이 변경되어야 한다.
    if($member['point'] >= $tot_price10){ //포인트보유액 >= 구매금액10% 
        if($tot_price10 > 10000){
            $de_useprice = 10000;
            $tot_price -= $de_useprice;
        }else {
            $de_useprice = $tot_price10;
            $tot_price -= $de_useprice;
        }
    }else {
        $de_useprice = $member['point'];  
        $tot_price -= $de_useprice;
    } 
...
<!-- 		</tr> -->
        <tr>
            ...
            <?php }else{ ?> 
            <td><input type="text" name="cellphone" value="<?php echo $member['cellphone']; ?>" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></td>
            <?php } ?>
        ...
        <tr>
            ...
            <td>
                ...
                <div class="padt8">
                    <input type="text" name="addr3" value="" class="frm_input" size="60" readonly> 참고항목
        ...
        <tr>
            <th scope="row">이메일</th>
            <td>
<!-- 	20200811이메일 셀렉추가로 폼변경 <label><input type="text" name="email" value="<?php echo $member['email']; ?>" required email itemname="E-mail" class="frm_input required" size="30"></label> -->
...
<!-- 		</tr> -->
        <tr>
            <th scope="row">휴대폰</th>
            <td><input type="text" name="b_cellphone" required itemname="핸드폰" class="frm_input required" onKeyup="PhoneNumberSplit(this);" size="20" maxlength="13"></td>
        </tr>
        <tr>
            <th scope="row">주소</th>
            <td>
...
if($default['de_card_use']) {
    echo '<input type="radio" name="paymethod" id="paymethod_card" value="신용카드" onclick="calculate_paymethod(this.value);" checked> <label for="paymethod_card">신용카드</label>'.PHP_EOL;
}
...
<?php
        ...
        <tr>
            ...
            <td>
                <input type="text" name="use_point" value="<?php echo $de_useprice;?>" class="frm_input" size="12" onkeyup="calculate_temp_point(this.value);this.value=number_format(this.value);" style="font-weight:bold;"> P 보유포인트 : <?php echo display_point($member['point']); ?>
                <?php if($config['usepoint']) { ?>

                    (<strong>1만포인트</strong> 이상 보유시 구매가의 10% 이내에서 최대 1만포인트까지 100P단위로 사용가능) 
                <?php } ?>
            </td>
        </tr>
        <?php } ?>
...
<section id="guest_privacy">
    <h3 class="pg_cnt marb15">
        <strong class="line">개인정보 수집 및 이용</strong>비회원으로 주문 시 포인트적립 및 추가 혜택을 받을 수 없습니다.
    </h3>
    <div class="tbl_head02 tbl_wrap">
        <table>
        <thead>
...
<div class="cart_r">
    ...
    <div class="btn_confirm">
        ...
        <!-- <a href="<?php echo TB_SHOP_URL; ?>/cart.php" class="btn_large bx-white">취소</a> -->
            ...
            <script>
            var totEl = document.buyform;
        console.log(typeof(totEl.tot_price.value));
    ...
    /*
        ...
        if(chk){
            document.buyform.use_point.value = 0;
            calculate_temp_point(0);
        }
     ...
     */
        ...
        function calculate_order_price() {
            ...
            //tot_price_ 포인트 반영_20191219
            $("input[name=tot_price_]").val(String(tot_price));

            //우측하단 적립금,결제예정금액 표시작업 필요
            $("#h_point").html(f.use_point.value + "원"); 
        ...
        function fbuyform_submit(f) {
            ...
            //구매가 10% ,최대사용금액설정
            var tot_price10 = parseInt(tot_price/10);
            var max_point = 10000;
            //alert('구매가10%' + tot_price10);
            ...
            //포인트 사용금액체크(구매금액 10%이내,최대1만포인트)
            if(temp_point > tot_price10 ) {
                alert('포인트 사용금액은 구매가의 10% 이내에서 가능합니다.');
                f.tot_price.value = number_format(String(tot_price));
                f.use_point.value = 0;
                f.use_point.focus();
                calculate_order_price(); 
            ...
            if(temp_point > max_point) {
                alert('포인트 사용금액은 최대 1만 포인트까지 가능합니다.');
                f.tot_price.value = number_format(String(tot_price));
                f.use_point.value = 0;
                f.use_point.focus();
                calculate_order_price(); 
        ...
        function calculate_temp_point(val) {
            ...
            if(val == '' || !checkNum(no_comma(val))) {
                ...
                $("#tot_price3").html(commaStr(tot_price) +"원");
                f.use_point.focus();
                return;
            }else {
                f.tot_price.value = number_format(String(tot_price - temp_point));
                f.tot_price_.value = tot_price - temp_point;//tot_price_ 반영_20191219
                //주문서 우측하단 포인트 반영_20191219
    ...
    $("#tot_price3").html(t_price + "원");
        ...
        function calculate_paymethod(type) {
            ...
            case '무통장':
                ...
                $(".bank_acc_tr").show();
                $(".tax_tr").show();
                //			$("input[name=use_point]").val(0); //20200616주석 결제수단변경시 포인트입력값 초기화 방지
                $("input[name=use_point]").attr("readonly", false);
        ...
        function point_notuse()
        {
            //alert(document.buyform.p_check.value);  
        }

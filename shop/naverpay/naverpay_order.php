...
include_once(TB_LIB_PATH.'/naverpay.lib.php');
...
define('NAVERPAY_BACK_URL', TB_SHOP_URL.$back_uri);
...
for($i=0; $i<$count; $i++) {
    ...
    //--------------------------------------------------------
    for($k=0; $k<$opt_count; $k++) {
        $io_id = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['io_id'][$gs_id][$k])));
...
$order = new naverpay_register($options, $ct_send_cost, $cart_cost);
$query = $order->query();

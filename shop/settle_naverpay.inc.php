...
if(!is_admin() && !$default['de_card_test'] && $default['de_naverpay_test']) {
    if($default['de_naverpay_mb_id'] && (!$is_member || $member['id'] != $default['de_naverpay_mb_id']))
        return;
}
...
function buy_nc(url)
{
    ...
    if( check ) {
        ...
        //f.action = "'.TB_SHOP_URL.'/naverpay/naverpay_order.php";
        //f.target = "win_buy_nc";
        //f.submit();
        //return false;

        $.ajax({
            url : "'.TB_SHOP_URL.'/naverpay/naverpay_order.php",
            type : "POST",
            data : $(f).serialize(),
            async : false,
            cache : false,
...
function wishlist_nc(url)
{
    ...
    f.action = "'.TB_SHOP_URL.'/naverpay/naverpay_wish.php";
    f.submit();
